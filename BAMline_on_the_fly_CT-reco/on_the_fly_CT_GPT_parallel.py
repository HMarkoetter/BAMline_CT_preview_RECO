...
class On_the_fly_CT_tester(Ui_on_the_fly_Window, Q_on_the_fly_Window):
    ...
    def check_batch_CORs(self):
        ...
        COR_in_list = numpy.where(self.batch_CORs == [self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round((self.extend_FOV_fixed_ImageJ_Stream) * self.full_size)])[0]
        ...
        if COR_in_list.size > 0:
            print('COR_in_list position: ', COR_in_list[0])
            print('COR already reconstructed. Display from RAM.')
            self.slice = self.recon_stack[COR_in_list[0],:,:]
                        
            # trim reconstructed slice
            if self.comboBox_180_360.currentText() == '180 - axis centered':
                self.slice = self.slice[round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size): -round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size),round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size): -round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size)]
                #slices = slices[:, : ,: ]
            else:
                self.slice = self.slice[round((self.extend_FOV_fixed_ImageJ_Stream -1) * self.full_size): -round((self.extend_FOV_fixed_ImageJ_Stream -1) * self.full_size),round((self.extend_FOV_fixed_ImageJ_Stream -1) * self.full_size): -round((self.extend_FOV_fixed_ImageJ_Stream -1) * self.full_size)]
            ...
            if self.radioButton_16bit_integer.isChecked():
                ...
                self.ima3 = numpy.clip((original_reconstruction * scale_factor - int_low_val) * inv_range , 0, 65535).astype(numpy.float32)
    ...
    def reconstruct(self):
        ...
        if self.comboBox_180_360.currentText() == '360 - axis right':
            self.center_list = [self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round((self.extend_FOV_fixed_ImageJ_Stream -1) * self.full_size)] * (self.number_of_used_projections)
            #self.center_list = [self.COR.value() +  self.full_size] * (self.number_of_used_projections)
        else:
            self.center_list = [self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size)] * (self.number_of_used_projections)
        ...
        if self.checkBox_phase_2.isChecked() == True:
            ...
            extended_sinos = tomopy.prep.phase.retrieve_phase(extended_sinos, pixel_size=0.0001*self.pixel_size.value(), dist=self.doubleSpinBox_distance_2.value(), energy=self.doubleSpinBox_Energy_2.value(), alpha=self.doubleSpinBox_alpha_2.value(), pad=True, ncore=None, nchunk=None)
        ...
        shifts = numpy.linspace(-((self.pre_reco-1)/4), ((self.pre_reco-1)/4), num=self.pre_reco)  # z.B. -5 up to +5 Pixel => pre_reco = 21; -31.5 up to +31.5 => pre_reco = 127
    ...
    def reconstruct_all(self):
        ...
        while (i < math.ceil((self.spinBox_last.value() - self.spinBox_first.value()) / self.block_size)):       #Need to fix the rest of slices
            ...
            extended_sinos = numpy.nan_to_num(tomopy.minus_log(self.Norm_vol[self.last_zero_proj : min(self.last_zero_proj + self.number_of_used_projections, self.Norm_vol.shape[0]), :, :]), copy=True, nan=1.0, posinf=1.0, neginf=1.0)
            extended_sinos = tomopy.misc.morph.pad(extended_sinos, axis=2, npad=round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size), mode='edge')
            end_time = time.time()
            ...
            self.center_list = [self.COR.value() + self.COR_roll.value() * (i + self.spinBox_first.value()) * self.block_size + round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size)] * (
            ...
            start_time = time.time()
            (spinBox_top,spinBox_bottom,spinBox_left,spinBox_right) = (self.spinBox_top.value(), self.spinBox_bottom.value(), self.spinBox_left.value(), self.spinBox_right.value())
            if any(v != 0 for v in (spinBox_top,spinBox_bottom,spinBox_left,spinBox_right)):
                slices = slices[:, spinBox_top : slices.shape[1] - spinBox_bottom, spinBox_left : slices.shape[2] - spinBox_right]
            end_time = time.time()
            ...
            if self.radioButton_16bit_integer.isChecked():
                inv_range =  65535.0 / range_val  # ← einmalige Division
                slices_save = numpy.clip((slices * scale_factor - int_low_val) * inv_range , 0, 65535).astype(numpy.uint16)
