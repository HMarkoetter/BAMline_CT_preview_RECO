...
class On_the_fly_CT_tester(Ui_on_the_fly_Window, Q_on_the_fly_Window):
    ...
    def __init__(self):
        ...
        self.center_list = [-1, 0]
        self.COR_batch_Offset_Angle = 0
    ...
    def check_test_button(self):
        # check what is still in RAM and does not need to be loaded
        if self.slice_in_ram != self.slice_number.value() or self.ringradius_in_RAM != self.spinBox_ringradius.value() or self.spinBox_DF_in_ram != self.spinBox_DF.value() or self.spinBox_back_illumination_in_ram != self.spinBox_back_illumination.value():
            ...
            self.check_batch_CORs()
        return

    def check_batch_CORs(self):
        # check what CORs are already in RAM and do not need to be reconstructed
        COR_in_list = numpy.where(self.batch_CORs == [
            self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round(
                (self.extend_FOV_fixed_ImageJ_Stream) * self.full_size)])[0]
        # print('comparing with batch_CORs: ', self.batch_CORs)
        # print('COR.value: ', self.COR.value())
        # print('COR extended FOV: ', self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round((self.extend_FOV_fixed_ImageJ_Stream) * self.full_size))
        ...
        if COR_in_list.size > 0:
            ...
            if self.comboBox_180_360.currentText() == '180 - axis centered':
                self.slice = self.slice[round(self.extend_FOV_fixed_ImageJ_Stream * self.full_size): -round(
                    self.extend_FOV_fixed_ImageJ_Stream * self.full_size), round(
                    self.extend_FOV_fixed_ImageJ_Stream * self.full_size): -round(
                    self.extend_FOV_fixed_ImageJ_Stream * self.full_size)]
                # slices = slices[:, : ,: ]
            else:
                self.slice = self.slice[round((self.extend_FOV_fixed_ImageJ_Stream - 1) * self.full_size): -round(
                    (self.extend_FOV_fixed_ImageJ_Stream - 1) * self.full_size), round(
                    (self.extend_FOV_fixed_ImageJ_Stream - 1) * self.full_size): -round(
                    (self.extend_FOV_fixed_ImageJ_Stream - 1) * self.full_size)]
            self.slice_size = self.slice.shape[1]

            self.slices = numpy.stack([self.slice] * 2, axis=0)

            self.slices = tomopy.circ_mask(self.slices, axis=0, ratio=1.0)

            self.slice = self.slices[0, :, :]

            if self.COR_batch_Offset_Angle != self.Offset_Angle.value():
                self.slice = rotate(self.slice, self.Offset_Angle.value() - self.COR_batch_Offset_Angle, axes=(1, 0),
                                    reshape=False, order=3, mode='constant', cval=0.0, prefilter=False)

            if self.checkBox_ruler.isChecked():
                self.add_ruler()

            # crop before sending to ImageJ     USER-CROP
            original_reconstruction = self.slice[
                self.spinBox_top.value():-self.spinBox_bottom.value() - 1, self.spinBox_left.value():-self.spinBox_right.value() - 1]
            ...
            if self.new == 1:
                self.pv_rec['dimension'] = [
                    {'size': original_reconstruction.shape[1], 'fullSize': original_reconstruction.shape[1],
                     'binning': 1},
                    {'size': original_reconstruction.shape[0], 'fullSize': original_reconstruction.shape[0],
                     'binning': 1}]
                self.new = 0
            ...
            # scaling µ taking into account the pixel size
            scale_factor = 10000.0 * (1.0 / self.pixel_size.value())

            # 16-bit integer conversion with low and high value
            if self.radioButton_16bit_integer.isChecked():
                ...
                self.ima3 = numpy.clip((original_reconstruction * scale_factor - int_low_val) * inv_range, 0,
                                       65535).astype(numpy.float32)

                # write result to pv
                self.pv_rec['value'] = ({'floatValue': self.ima3.flatten()},)

            # keep 32-bit float with real µ values
            if self.radioButton_32bit_float.isChecked() == True:
                self.slice_show = original_reconstruction * scale_factor
    ...
    def cut_path_name(self):
        ...
        print('set Minimum')
        time.sleep(1)
        self.slice_number.setValue(round(self.vol_proxy.shape[
                                             1] / 2))  # be careful with an infinite loop when setValue actually triggers valueChanged. Therefore, auto_update starts off
    ...
    def load(self):
        ...
        if self.FF_before_after_checkbox.isChecked():
            ...
            Sino = self.vol_proxy[
                self.spinBox_number_FFs.value():-self.spinBox_number_FFs.value(), self.slice_number.value(), :]
        ...
        print('FFs for normalization ', self.spinBox_number_FFs.value(), FFmean.shape)
        self.Norm = numpy.divide(numpy.subtract(Sino, self.spinBox_DF.value()), numpy.subtract(FFmean,
                                                                                               self.spinBox_DF.value() + self.spinBox_back_illumination.value()))
        ...
        if self.spinBox_ringradius.value() != 0:
            ...
            proj_sum_filtered = median_filter(self.proj_sum_2d, size=(1, self.spinBox_ringradius.value()),
                                              mode='nearest')
            ...
            print('correction_map dimensions', correction_map.shape, 'correction_map min vs max',
                  numpy.amin(correction_map), numpy.amax(correction_map))
        ...
        poly_coeff = numpy.polyfit(
            numpy.arange(len(self.w[round((self.w.shape[0] + 1) / 4): round((self.w.shape[0] + 1) * 7 / 8)])),
            self.w[round((self.w.shape[0] + 1) / 4): round((self.w.shape[0] + 1) * 7 / 8)], 1, rcond=None, full=False,
            w=None, cov=False)
    ...
    def reconstruct(self):
        ...
        if self.graph[-1] >= 350:
            ...
            print('180°')
        print('number of projections used for reconstruction (omitting those above 180°/360°: )',
              self.number_of_used_projections)

        # create list with all projection angles
        new_list = (numpy.arange(
            self.number_of_used_projections) * self.speed_W.value() + self.Offset_Angle.value()) * math.pi / 180

        # create list with COR-positions of projections
        if self.comboBox_180_360.currentText() == '360 - axis right':
            self.center_list = [self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round(
                (self.extend_FOV_fixed_ImageJ_Stream - 1) * self.full_size)] * (self.number_of_used_projections)
            # self.center_list = [self.COR.value() +  self.full_size] * (self.number_of_used_projections)
        else:
            self.center_list = [self.COR.value() + self.COR_roll.value() * self.slice_number.value() + round(
                self.extend_FOV_fixed_ImageJ_Stream * self.full_size)] * (self.number_of_used_projections)

        # create one sinogram in the form [z, y, x]
        transposed_sinos = numpy.zeros((min(self.number_of_used_projections, self.Norm.shape[0]), 1, self.full_size),
                                       dtype=float)
        transposed_sinos[:, 0, :] = self.Norm[
            self.last_zero_proj: min(self.last_zero_proj + self.number_of_used_projections, self.Norm.shape[0]), :]

        # extend data with calculated parameter, compute logarithm, remove NaN-values
        extended_sinos = tomopy.minus_log(transposed_sinos)
        extended_sinos = numpy.nan_to_num(extended_sinos, copy=True, nan=1.0, posinf=1.0, neginf=1.0)
        extended_sinos = tomopy.misc.morph.pad(extended_sinos, axis=2,
        ...
        # apply phase retrieval if checked
        if self.checkBox_phase_2.isChecked() == True:
            print('applying phase contrast')
            extended_sinos = tomopy.prep.phase.retrieve_phase(extended_sinos,
                                                              pixel_size=0.0001 * self.pixel_size.value(),
        ...
        self.COR_batch_Offset_Angle = self.Offset_Angle.value()
    ...
    def add_ruler(self):
        ...
        cv2.circle(self.slice, (round(self.slice.shape[1] / 2), round(self.slice.shape[1] / 2)), 1280, (255, 255, 255),
                   4)
        ...
        if self.grid_micrometer.isChecked() == True:
            ...
            for r in range(round(self.slice.shape[1] * self.pixel_size.value() / 2),
                ...
                cv2.putText(self.slice, str(round(1000 * self.piezo_45_proxy[-1] / 5) * 5 + round(r / 5) * 5 - round(
                    self.slice.shape[1] * (self.pixel_size.value() / 10)) * 5),
        ...
        if self.grid_pixel.isChecked() == True:
            ...
            for r in range(round(self.slice.shape[1] / 2), self.slice.shape[1], round(self.spinBox_pixel_grid.value())):
                ...
                cv2.putText(self.slice, str(round((r - (self.slice.shape[1] / 2)) / 5) * 5), (r + 20, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.750, (65535, 65535, 65535), thickness=2)
            ...
            for r in range(round(self.slice.shape[1] / 2), self.slice.shape[1], round(self.spinBox_pixel_grid.value())):
                ...
                cv2.putText(self.slice, str(round((r - (self.slice.shape[1] / 2)) / 5) * 5), (20, r + 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.750, (65535, 65535, 65535), thickness=2)
    ...
    def reconstruct_all(self):
        ...
        self.path_out_reconstructed_ask = QtWidgets.QFileDialog.getExistingDirectory(self,
        ...
        with open(file_name_parameter, mode='w', newline='') as parameter_file:
            ...
            csv_writer.writerow(
                ['16-bit                        ', str(self.radioButton_16bit_integer.isChecked()), ' '])
        ...
        start_time_allblocks = time.time()
        i = 0
        while (i < math.ceil((
                                     self.spinBox_last.value() - self.spinBox_first.value()) / self.block_size)):  # Need to fix the rest of slices
            ...
            if self.spinBox_ringradius.value() != 0:
                ...
                proj_sum_filtered = median_filter(self.proj_sum, size=(1, self.spinBox_ringradius.value()),
                                                  mode='nearest')
            ...
            extended_sinos = numpy.nan_to_num(tomopy.minus_log(self.Norm_vol[self.last_zero_proj: min(
                self.last_zero_proj + self.number_of_used_projections, self.Norm_vol.shape[0]), :, :]), copy=True,
            ...
            if self.algorithm_list.currentText() == 'FBP_CUDA':
                options = {'proj_type': 'cuda', 'method': 'FBP_CUDA'}
                slices = tomopy.recon(extended_sinos, new_list, center=self.center_list, algorithm=tomopy.astra,
                                      options=options)
            else:
                slices = tomopy.recon(extended_sinos, new_list, center=self.center_list,
                                      algorithm=self.algorithm_list.currentText(),
                                      filter_name=self.filter_list.currentText())
            end_time = time.time()
            ...
            start_time = time.time()
            (spinBox_top, spinBox_bottom, spinBox_left, spinBox_right) = (self.spinBox_top.value(),
                                                                          self.spinBox_bottom.value(),
                                                                          self.spinBox_left.value(),
                                                                          self.spinBox_right.value())
            if any(v != 0 for v in (spinBox_top, spinBox_bottom, spinBox_left, spinBox_right)):
                slices = slices[
                    :, spinBox_top: slices.shape[1] - spinBox_bottom, spinBox_left: slices.shape[2] - spinBox_right]
            end_time = time.time()
            ...
            if self.radioButton_16bit_integer.isChecked():
                inv_range = 65535.0 / range_val  # ← einmalige Division
                slices_save = numpy.clip((slices * scale_factor - int_low_val) * inv_range, 0, 65535).astype(
                    numpy.uint16)
            ...
            if self.save_tiff.isChecked() == True:
                ...
                for a in range(min(self.block_size, slices_save.shape[0])):
                    ...
                    img = Image.fromarray(slices_save[a, :, :])
                    img.save(self.path_out_reconstructed_full + self.namepart + '_' + str(
            ...
            if self.save_hdf5.isChecked() == True:
                if i == 0:
                    ...
                    with h5py.File(self.path_klick, 'r') as f1, h5py.File(
                            self.path_out_reconstructed_full + '/' + self.folder_name + '_reco' + '.h5', 'w') as f:
                        ...
                        f.create_dataset("Volume", dtype='uint16', data=slices_save,
                                         chunks=(self.block_size, self.hdf_chunking_size_y, self.hdf_chunking_size_x),
                                         maxshape=(None, slices_save.shape[1], slices_save.shape[2]))
            ...
            print("block duration: {:.3f} seconds".format(end_time_block - start_time_block))

        end_time_allblocks = time.time()
        print("All blocks duration: {:.3f} seconds".format(end_time_allblocks - start_time_allblocks))
