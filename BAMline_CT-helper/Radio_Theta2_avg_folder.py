...
standard_path = "/raid/CT/2023/2023_03/Markoetter/daisy_lam3_top_mystery/"
...
class Radio_Theta2_avg(Ui_Theta2_avg_Window, Q_Theta2_avg_Window):
    ...
    def load(self):
        ...
        # write result to pv
        self.pv_rec['value'] = ({'doubleValue': self.Norm_stack.flatten()},)
    ...
    def compute(self):
        ...
        while i < self.min_size:
            ...
            self.Norm_stack2[i,:,:] = scipy.ndimage.shift(self.Norm_stack[i,:,:], (i * self.doubleSpinBox_Shift.value(),0), order=self.intSpinbox_Interpolationorder.value(), mode='nearest', prefilter=True)
            i=i+1
        ...
        self.stdtext.setText(str(numpy.std(self.avg)))
        self.blurtext.setText(str(skimage.measure.blur_effect(self.avg, h_size=11)))
