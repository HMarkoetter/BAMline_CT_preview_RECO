...
Ui_CorrectionWindow, QCorrectionWindow = loadUiType('movement_correction.ui')  # GUI vom Hauptfenster
...
class Movement_corrector(Ui_CorrectionWindow, QCorrectionWindow):
    ...
    def analyze(self):
        ...
        self.step_size = self.step_size.currentText()
        if self.step_size == '1':
            stp = 1
            pts = 1
            print('Step Size ',stp)
        ...
        print('trying to create numpy zeros', 2 * scan_range * pts + 1)
        results_ = numpy.zeros(2 * scan_range * pts + 1)

        print('trying to create numpy zeros', self.number_of_projections)
    ...
    def reconstruct(self):
        ...
        center_list = [self.doubleSpinBoxCOR.value()] * (self.number_of_projections)
        print(center_list)

        sinos = numpy.zeros((3, self.full_size, self.number_of_projections), dtype=float)
        sinos[0,:,:] = self.sino
        sinos[1,:,:] = self.sino
        sinos[2,:,:] = self.sino
        ...
        corrected_sinos = numpy.zeros((3, self.full_size, self.number_of_projections), dtype=float)
        corrected_sinos[0,:,:] = self.corrected_sinogram
        corrected_sinos[1,:,:] = self.corrected_sinogram
        corrected_sinos[2,:,:] = self.corrected_sinogram
        transposed_correted_sinos = numpy.transpose(corrected_sinos, axes = [2,0,1])
        print('transposed_corrected_sinos_shape', transposed_correted_sinos.shape)

        transposed_sinos = tomopy.minus_log(transposed_sinos)
        transposed_correted_sinos = tomopy.minus_log(transposed_correted_sinos)


        corrected_slices = tomopy.recon(transposed_correted_sinos, new_list, center=center_list, algorithm='gridrec',
                              filter_name='shepp', ncore=14)
        corrected_slices = tomopy.circ_mask(corrected_slices, axis=0, ratio=1.0)

        original_slices = tomopy.recon(transposed_sinos, new_list, center=center_list, algorithm='gridrec',
                              filter_name='shepp', ncore=14)
        original_slices = tomopy.circ_mask(original_slices, axis=0, ratio=1.0)

        print('reconstructions done')
        print(corrected_slices.shape)
        print(original_slices.shape)

        corrected_reconstruction = corrected_slices[1,:,:]
        original_reconstruction = original_slices[1,:,:]
        print('cut reconstructions')
        print(corrected_reconstruction.shape)
        print(original_reconstruction.shape)
